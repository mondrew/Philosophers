...
/*   Updated: 2020/11/26 22:16:05 by mondrew          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers.h"

void	ft_philosophers_two(t_data *philo, int i)
{
	int			thread_num_array[philo->number_of_philosophers];
	int			block_at_start[philo->number_of_philosophers];
	int			eat_times[philo->number_of_philosophers];
	...
	ft_start_init(philo, thread_num_array, eat_times, prev_eat);
	...
	{
		if ((pthread_create(&pthread[i], NULL, ft_philo_life, \
														(void *)philo)) != 0)
